    ...
    return opportunities
      .filter((opportunity) => {
        const matchesCategory =
          !filters.category || opportunity.category.toLowerCase().includes(filters.category.toLowerCase());
        const matchesTags =
          !filters.tags ||
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr,2fr]">
        <form onSubmit={submit} className="space-y-3 rounded-lg bg-white p-4 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            <label>
              Demand fit
              <input type="number" min={0} max={10} value={form.demandFit} onChange={(e) => setForm({ ...form, demandFit: Number(e.target.value) })} />
            </label>
            <label>
              Budget
              <input type="number" min={0} max={10} value={form.budgetPotential} onChange={(e) => setForm({ ...form, budgetPotential: Number(e.target.value) })} />
            </label>
            <label>
              Execution
              <input type="number" min={0} max={10} value={form.executionConfidence} onChange={(e) => setForm({ ...form, executionConfidence: Number(e.target.value) })} />
            </label>
          </div>
        ...
        <div className="space-y-3">
              ...
              <li key={opportunity.id} className="rounded-lg bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="flex gap-2 text-sm">
                    <button className="bg-slate-100" onClick={() => setSelectedId(opportunity.id)} type="button">Details</button>
                    <button className="bg-slate-100" onClick={() => onEdit(opportunity)} type="button">Edit</button>
                    <button
          ...
          {selected ? (
            <aside className="rounded-lg border border-slate-300 bg-white p-4">
              ...
              <p className="text-sm">
                Breakdown: {selected.scoreBreakdown.demandFit}/{selected.scoreBreakdown.budgetPotential}/
                {selected.scoreBreakdown.executionConfidence}
              </p>
            </aside>
          ) : null}
        </div>
